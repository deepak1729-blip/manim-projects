...
def glow_vmobject(vmob, color=None, layers=3, base_width=8, opacity=0.22):
    if color is None:
        color = vmob.get_color()
    halos = VGroup()
    for i in range(layers):
        halo = vmob.copy()
        halo.set_fill(opacity=0)
        halo.set_stroke(color, width=base_width * (i + 1),
                        opacity=opacity / (i + 1))
        halos.add(halo)
    halos.set_z_index(vmob.get_z_index() - 1)
    return VGroup(halos, vmob)
...
class Scene6_TheTakeaway(Scene):
    def construct(self):
        ...
        big_box_glow = glow_vmobject(big_box, COLOR_BLUE_BALL,
                                     layers=3, base_width=7, opacity=0.20)
        ...
        small_v_glow = glow_vmobject(small_v[0], COLOR_VEC_G,
                                     layers=3, base_width=6, opacity=0.18)
        small_h_glow = glow_vmobject(small_h[0], COLOR_VEC_V_X,
                                     layers=3, base_width=6, opacity=0.18)

        self.play(
            flash_line.animate.set_stroke(width=0, opacity=0).scale(3),
            FadeOut(big_box_glow, scale=0.8),
        ...
        self.play(
            FadeIn(small_v_glow), FadeIn(small_h_glow),
        ...
        small_v_glow[0].add_updater(box_breath_v)
        small_h_glow[0].add_updater(box_breath_h)

        self.wait(2.0)

        small_v_glow[0].clear_updaters()
        small_h_glow[0].clear_updaters()

        act3_all = VGroup(
            big_02, p2_top,
            small_v, small_h, small_v_glow, small_h_glow
        )
        self.play(
